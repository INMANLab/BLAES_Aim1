%% Create by Krista Wahlstrom 2023
%Determine patient behavioral responses for the post test session
%stimulation awareness test for BLAES Aim1 long-delay
...
d = dir(fullfile(cd,'data',subjID,'BLAES_PostSessionStimTest','*.dat'));
...
collectStimDataRowCount = 1;
    ...
    save(fullfile(cd,'data',subjID,'BLAES_PostSessionStimTest',strcat(subjID,'_StimTest_Data', '.mat')),'collectStimData')
...
title([subjID, ' ', 'Stimulation Awareness Test Responses'],'fontweight','bold','fontsize',16)
...
%Save bar graph
savefile = fullfile(cd, 'figures', subjID, strcat(subjID, '_StimTest_Responses.png'));
saveas(f, savefile);