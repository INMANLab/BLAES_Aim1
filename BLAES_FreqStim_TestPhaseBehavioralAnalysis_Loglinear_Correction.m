%Krista Wahlstrom, 9/14/23
% This is the behavioral analysis script for BLAES Aim 1 Frequency Stimulation Task (8x80Hz stim, 8x50Hz stim, and no stim conditions) for the Test
...
function BLAES_FreqStim_TestPhaseBehavioralAnalysis_Loglinear_Correction()
...
% 2702 Stim 8x50Hz (only applies to the STUDY PHASE)
% 2703 Stim 8x80Hz (only applies to the STUDY PHASE)
...
%% Behavioral Analysis (for no stim compared to 8x50Hz stim condition)
...
title([subjID, ' ', RetrievalDelay ' Hit Rate - Combined 8x50Stim/NoStim'])
set(gca,'XTick',1,'XTickLabel',{'8x50Stim/NoStim'})
axis([0.5 2.0 0 1])

fprintf('Saving trained images - combined stim figure...\n')
savefile = fullfile(cd, 'figures', subjID, strcat(subjID, '_', RetrievalDelay, SureResponseString,'_Loglinear', '_TrainedImagesCombined8x50Stim.png'));
...
set(gca,'XTick',[1 2],'XTickLabel',{'NoStim','8x50Stim'})
ylabel('Occurrences (% of Total)')
set(gca,'YTick',0:0.25:1,'YTickLabel',100*round(0:0.25:1,2))
set(gca,'FontName','Arial','FontSize',24,'LineWidth',2,'Box','off')
axis([0.5 3.0 0 1])
...
savefile = fullfile(cd, 'figures', subjID, strcat(subjID, '_', RetrievalDelay, SureResponseString,'_Loglinear', '_8x50stim_TrainedImages.png'));
...
%     hatchfill2(object_h(4), 'cross','HatchAngle',45,'HatchDensity',40/4,'hatchcolor',[1 0 0]);
    ...
    fprintf('Saving trained images figure...\n')
    savefile = fullfile(cd, 'figures', subjID, strcat(subjID, '_', RetrievalDelay, SureResponseString,'_Loglinear', '_8x50stim_ActualHitRateSURE.png'));
...
%% d prime 
% Breakout 8x50hz stim, no stim (includes loglinear correction)
...
clabelstring    = {'objects nostim', 'objects 8x50stim'};
...
savefile = fullfile(cd, 'figures', subjID, strcat(subjID, '_', RetrievalDelay, SureResponseString,'_Loglinear', '_8x50stim_dprime.png'));
...
if size(ResponseKeys,2) == 2
    ...
    fprintf('Saving d prime figure...\n')
    savefile = fullfile(cd, 'figures', subjID, strcat(subjID, '_', RetrievalDelay, SureResponseString,'_Loglinear', '_8x50stim_ActualSuredprime.png'));
...
%% d prime - combined 8x50hz stim and no stim
...
set(gca,'FontName','Arial','FontSize',24,'LineWidth',2,'Box','off')
axis([0.5 4.5 ylim])
title([subjID, ' ', RetrievalDelay ' Discrimination Index - Combined 8x50Stim/NoStim (out of total images)'])

fprintf('Saving d prime with combined stim figure...\n')
savefile = fullfile(cd, 'figures', subjID, strcat(subjID, '_', RetrievalDelay, SureResponseString,'_Loglinear', '_dprimeCombined8x50Stim.png'));
...
%% d prime - combined 8x50hz stim and no stim (ACTUAL dprime -- Sure only for total number of "sure" images)
...
if size(ResponseKeys,2) == 2
    ...
    set(gca,'FontName','Arial','FontSize',24,'LineWidth',2,'Box','off')
    axis([0.5 4.5 ylim])
    title([subjID, ' ', RetrievalDelay ' Actual Discrimination Index - Combined 8x50Stim/NoStim (out of sure only responses)'])
    
    fprintf('Saving d prime with combined stim figure...\n')
    savefile = fullfile(cd, 'figures', subjID, strcat(subjID, '_', RetrievalDelay, SureResponseString,'_Loglinear', '_ActualSuredprimeCombined8x50Stim.png'));
...
%% Behavioral Analysis (for no stim compared to 8x80Hz stim condition)
% hit/miss/false alarm/correct rejection
% Yes (targ/old) No (New)       Sure           Not Sure
% KeyDown == 37  KeyDown == 39  KeyDown == 37  KeyDown == 39
...
% Get total number of possible HITS for the 8x80stim condition (all other
% response condition counts from above remain the same)
TotalObjectHit8x80Stim    = GetBA_MaxPossible(collectData, 'Targ',  2);
...
for i = 1:size(collectData,1)
    ...
    % Get counts for patient responses related to the 8x80hz stim condition
    
    ObjectHit_8x80Stim(i,1)     = CheckBAConditions(collectData(i,:), 'Targ',  2, YesResponse);  
    
    ObjectMiss_8x80Stim(i,1)    = CheckBAConditions(collectData(i,:), 'Targ',  2, NoResponse);
...
%%  Total hits across all categories
% combining stim and nostim hits for object images
labelstring = {'NoStim + 8x80Stim'};
textStartY = 0.75;
textStepY  = 0.05;

TrainedImagesCombinedStimFig = figure('Position',figsize);
b = bar((sum(ObjectHit_NoStim + ObjectHit_8x80Stim))/...
    (TotalObjectHitNoStim + TotalObjectHit8x80Stim));
b(1).FaceColor = [0.4940 0.1840 0.5560]; % purple
text(1.55,textStartY,'number of hits:','FontSize',18,'fontweight','bold')
text(1.55,textStartY-1*textStepY,labelstring,'FontSize',18)
text(1.8,textStartY-1*textStepY,num2str(sum(ObjectHit_NoStim + ObjectHit_8x80Stim)),'FontSize',18)
ylabel('Occurrences (% of Total)')
set(gca,'YTick',0:0.25:1,'YTickLabel',100*round(0:0.25:1,2))
set(gca,'FontName','Arial','FontSize',24,'LineWidth',2,'Box','off')
title([subjID, ' ', RetrievalDelay ' Hit Rate - Combined 8x80Stim/NoStim'])
set(gca,'XTick',1,'XTickLabel',{'8x80Stim/NoStim'})
axis([0.5 2.0 0 1])

fprintf('Saving trained images - combined stim figure...\n')
savefile = fullfile(cd, 'figures', subjID, strcat(subjID, '_', RetrievalDelay, SureResponseString,'_Loglinear', '_8x80TrainedImagesCombinedStim.png'));
...
TrainedImagesFig = figure('Position',figsize);
b = bar([((sum(ObjectHit_NoStim))/(TotalObjectHitNoStim));    ((sum(ObjectHit_8x80Stim))/(TotalObjectHit8x80Stim))],'FaceColor','Flat');
...
text(2.8,textStartY-1*textStepY,num2str(sum(ObjectHit_NoStim)),'FontSize',18)
text(2.8,textStartY-2*textStepY,num2str(sum(ObjectHit_8x80Stim)),'FontSize',18)

set(gca,'XTick',[1 2],'XTickLabel',{'NoStim','8x80Stim'})
ylabel('Occurrences (% of Total)')
set(gca,'YTick',0:0.25:1,'YTickLabel',100*round(0:0.25:1,2))
set(gca,'FontName','Arial','FontSize',24,'LineWidth',2,'Box','off')
axis([0.5 3.0 0 1])
...
fprintf('Saving trained images figure...\n')
savefile = fullfile(cd, 'figures', subjID, strcat(subjID, '_', RetrievalDelay, SureResponseString,'_Loglinear', '_8x80stim_TrainedImages.png'));
...
if size(ResponseKeys,2) == 2
    ...
    ActualHitRateSureFig = figure('Position',figsize);
    b = bar([((sum(ObjectHit_NoStim))/(sum(ObjectHit_NoStim) + sum(ObjectMiss_NoStim)));    ((sum(ObjectHit_8x80Stim))/(sum(ObjectHit_8x80Stim) + sum(ObjectMiss_8x80Stim)))],'FaceColor','Flat');
    ...
    text(2.8,textStartY-1*textStepY,strcat(num2str(sum(ObjectHit_NoStim)),' (out of ',num2str(sum(ObjectHit_NoStim) + sum(ObjectMiss_NoStim)), ')'),'FontSize',18)
    text(2.8,textStartY-2*textStepY,strcat(num2str(sum(ObjectHit_8x80Stim)),' (out of ',num2str(sum(ObjectHit_8x80Stim) + sum(ObjectMiss_8x80Stim)), ')'),'FontSize',18)
...
%     hatchfill2(object_h(4), 'cross','HatchAngle',45,'HatchDensity',40/4,'hatchcolor',[1 0 0]);
    ...
    fprintf('Saving trained images figure...\n')
    savefile = fullfile(cd, 'figures', subjID, strcat(subjID, '_', RetrievalDelay, SureResponseString,'_Loglinear', '_8x80stim_ActualHitRateSURE.png'));
...
[dp(1,1), c(1)] = dprime_simple((sum(ObjectHit_NoStim) + 0.5)/(TotalObjectHitNoStim + 1),(sum(ObjectFalseAlarm) + 0.5)/(TotalObjectFA + 1));         % nostim 
[dp(1,2), c(2)] = dprime_simple((sum(ObjectHit_8x80Stim) + 0.5)/(TotalObjectHit8x80Stim + 1),(sum(ObjectFalseAlarm) + 0.5)/(TotalObjectFA + 1));             % stim 
...
clabelstring    = {'objects nostim', 'objects 8x80stim'};
textStartY = max(max(dp,[],'omitnan')) - 0.15*max(max(dp,[],'omitnan'));
textStepY  = max(max(dp,[],'omitnan'))/20;
...
%Create plot text that lists the difference in dprime for stim vs. nostim
plottext1 = {'Dprime Diff: '};
plottext2 = {strcat('  ',num2str(abs(dprime_simple((sum(ObjectHit_NoStim) + 0.5)/(TotalObjectHitNoStim + 1),(sum(ObjectFalseAlarm) + 0.5)/(TotalObjectFA + 1))-dprime_simple((sum(ObjectHit_8x80Stim) + 0.5)/(TotalObjectHit8x80Stim + 1),(sum(ObjectFalseAlarm) + 0.5)/(TotalObjectFA + 1))),3))};
...
fprintf('Saving d prime figure...\n')
savefile = fullfile(cd, 'figures', subjID, strcat(subjID, '_', RetrievalDelay, SureResponseString,'_Loglinear', '_8x80stim_dprime.png'));
...
if size(ResponseKeys,2) == 2
    ...
    [dp(1,1), c(1)] = dprime_simple((sum(ObjectHit_NoStim) + 0.5)/(sum(ObjectHit_NoStim) + sum(ObjectMiss_NoStim) + 1),(sum(ObjectFalseAlarm) + 0.5) /(sum(ObjectFalseAlarm) + sum(ObjectRejection) + 1));         % nostim 
    [dp(1,2), c(2)] = dprime_simple((sum(ObjectHit_8x80Stim) + 0.5)/(sum(ObjectHit_8x80Stim) + sum(ObjectMiss_8x80Stim) + 1),(sum(ObjectFalseAlarm) + 0.5)/(sum(ObjectFalseAlarm) + sum(ObjectRejection) + 1));             % stim 
    ...
    %Create plot text that lists the difference in dprime for stim vs. nostim
    plottext1 = {'Dprime Diff: '};
    plottext2 = {strcat('  ',num2str(abs(dprime_simple((sum(ObjectHit_NoStim) + 0.5)/(sum(ObjectHit_NoStim) + sum(ObjectMiss_NoStim) + 1),(sum(ObjectFalseAlarm) + 0.5) /(sum(ObjectFalseAlarm) + sum(ObjectRejection) + 1))-dprime_simple((sum(ObjectHit_8x80Stim) + 0.5)/(sum(ObjectHit_8x80Stim) + sum(ObjectMiss_8x80Stim) + 1),(sum(ObjectFalseAlarm) + 0.5)/(sum(ObjectFalseAlarm) + sum(ObjectRejection) + 1))),3))};
    ...
    fprintf('Saving d prime figure...\n')
    savefile = fullfile(cd, 'figures', subjID, strcat(subjID, '_', RetrievalDelay, SureResponseString,'_Loglinear', '_8x80stim_ActualSuredprime.png'));
...
%% d prime - combined 8x80stim and no stim
[dp_CombinedStim(1,1), c_CombinedStim(1)] = dprime_simple((sum(ObjectHit_NoStim + ObjectHit_8x80Stim) + 0.5)/...
                                                (TotalObjectHitNoStim + TotalObjectHit8x80Stim + 1),(sum(ObjectFalseAlarm) + 0.5)/(TotalObjectFA + 1));           % item
...
set(gca,'FontName','Arial','FontSize',24,'LineWidth',2,'Box','off')
axis([0.5 4.5 ylim])
title([subjID, ' ', RetrievalDelay ' Discrimination Index - Combined 8x80Stim/NoStim (out of total images)'])

fprintf('Saving d prime with combined stim figure...\n')
savefile = fullfile(cd, 'figures', subjID, strcat(subjID, '_', RetrievalDelay, SureResponseString,'_Loglinear', '_dprimeCombined8x80Stim.png'));
...
%% d prime - combined 8x80stim and no stim (ACTUAL dprime -- Sure only for total number of "sure" images)
...
if size(ResponseKeys,2) == 2
    ...
    [dp_CombinedStim(1,1), c_CombinedStim(1)] = dprime_simple((sum(ObjectHit_NoStim + ObjectHit_8x80Stim) + 0.5)/...
                                                    (sum(ObjectHit_NoStim + ObjectHit_8x80Stim) + sum(ObjectMiss_NoStim + ObjectMiss_8x80Stim) + 1),(sum(ObjectFalseAlarm) + 0.5)/(sum(ObjectFalseAlarm) + sum(ObjectRejection) + 1));     
    ...
    set(gca,'FontName','Arial','FontSize',24,'LineWidth',2,'Box','off')
    axis([0.5 4.5 ylim])
    title([subjID, ' ', RetrievalDelay ' Actual Discrimination Index - Combined 8x80Stim/NoStim (out of sure only responses)'])
    
    fprintf('Saving d prime with combined stim figure...\n')
    savefile = fullfile(cd, 'figures', subjID, strcat(subjID, '_', RetrievalDelay, SureResponseString,'_Loglinear', '_ActualSuredprimeCombined8x80Stim.png'));
